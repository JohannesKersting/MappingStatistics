...
directory <- "results"
mappers <- c("star","hisat","crac","contextmap2","BBmap","subjunc","dart","gsnap","mapsplice2","segemehl")
error_rate <- 0
samples <- c("subsample_01")
...
```{r}
ggplot(summary,aes(x=n_correct_range_pairs/n_proper_pairs,y=n_correct_range_pairs/n_pairs,color=mapper))+
  geom_point(size=3)+
...
```{r}
ggplot(summary,aes(x=n_all_junctions_correct_pairs/n_proper_pairs,y=n_all_junctions_correct_pairs/n_pairs,color=mapper))+
  geom_point(size=3)+
  labs(x="Correct pairs / mapped pairs",y="Correct pairs / all pairs",title=paste("Error rate:",error_rate,"Level: Read pairs with all junctions mapped correctly"),color="Mapping tool")+
  theme_bw()+
  theme(plot.title = element_text(size=13.5))
```
```{r}
...
ggplot(summary,aes(x=precision(n_correct_junctions,n_false_junctions),
                   ...
                   color=mapper))+
  geom_point(size=3)+
  labs(x="Precision",y="Recall",title=paste("Error rate:",error_rate,"Level: Junctions with correct range"),color="Mapping tool")+
  theme_bw()+
  theme(plot.title = element_text(size=13.5))
```
Make data long...
